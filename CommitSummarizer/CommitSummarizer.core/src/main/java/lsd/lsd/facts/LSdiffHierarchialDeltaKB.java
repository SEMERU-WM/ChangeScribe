...
import lsd.lsd.io.LSDTyrubaFactReader;
import lsd.lsd.rule.LSDBinding;
import lsd.lsd.rule.LSDConst;
...
//		File deltaKBFile = new File ("input/jfreechart/0.9.10_0.9.11delta.rub");
	...
	private TreeSet<LSDFact> expandTypeLevelCluster2TypeDependencies (PrintStream p, List<LSDFact> typeLevelCluster) { 
		...
		for (LSDFact fact:originalDeltaKB) {
			...
			if (fact != null && fact.getPredicate().getName().indexOf("_typeintype")>0) { 
				involvedType= fact.getBindings().get(1).getGroundConst(); 
			}
			// extends (Super, Sub)  => Sub was modified 
			else if (fact != null && fact.getPredicate().getName().indexOf("_extends")>0) { 
				involvedType = fact.getBindings().get(1).getGroundConst(); 
				// implements (Super, Sub) => Sub was modified
			}else if (fact != null && fact.getPredicate().getName().indexOf("_implements")>0) { 
				involvedType= fact.getBindings().get(1).getGroundConst(); 
			// FIXME: I want to move inherited files and inherited methods to method level or field level change.  
			}else if (fact != null && fact.getPredicate().getName().indexOf("_inheritedfield")>0) { 
				involvedType = fact.getBindings().get(2).getGroundConst(); 
			}else if (fact != null && fact.getPredicate().getName().indexOf("_inheritedmethod")>0) { 
				involvedType= fact.getBindings().get(2).getGroundConst();
			}
	...
	private TreeSet<LSDFact> expandTypeLevelCluster2Methods (PrintStream p, List<LSDFact> typeLevelCluster) { 	
		...
		for (LSDFact fact : originalDeltaKB) { 
			if (fact != null && fact.getPredicate().getName().indexOf("_return") > 0) { 
		...
		for (LSDFact fact : originalDeltaKB) {
			if (fact != null) {
				...
				if (predName.equals("added_return")
					...
					List<LSDBinding> bindings = fact.getBindings();
					LSDBinding firstBinding = bindings.get(0);
					LSDFact mmethod = LSDConst
							.createModifiedMethod(firstBinding.getGroundConst());
					if (!containsTheSameFact(addedMethod, deletedMethod,
							mmethod))
					...
					LSDFact mmethod = LSDConst
							.createModifiedMethod(secondBinding
									.getGroundConst());
					if (!containsTheSameFact(addedMethod, deletedMethod,
							mmethod))
						modifiedMethod.add(mmethod);
		...
		for (LSDFact fact : originalDeltaKB) {
			if (fact != null) {
				...
				if (predName.endsWith("_typeintype")) {
					...
					List<LSDBinding> bindings = fact.getBindings();
					LSDBinding secondBinding = bindings.get(1);
					LSDFact mtype = LSDConst.createModifiedType(secondBinding
							.getGroundConst());
					if (!containsTheSameFact(addedType, deletedType, mtype))
						modifiedType.add(mtype);
				} else if (predName.endsWith("_extends")
						|| predName.endsWith("_implements")) {
					...
					LSDFact m2type = LSDConst.createModifiedType(thirdBinding
							.getGroundConst());
