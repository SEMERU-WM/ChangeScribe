		...
		for (LSDFact twoKBfact : original2KB) {
			if (twoKBfact != null) {
				...
				if (tk_predName.endsWith("_accesses")
					...
					java.util.List<LSDBinding> bindings = twoKBfact
							.getBindings();
					for (int i = 0; i < types.length; i++) {
						// for each binding
				...
				// (A, ? ,B) => use A+B
				{
					...
					String fullName = LSDConst.createFullMethodOrFieldName(
							tk_binding_A.getGroundConst(),
							tk_binding_B.getGroundConst());
					LSDBinding tk_binding = LSDConst
							.createModifiedField(fullName).getBindings().get(0);// create a new binding
					char[] types = tk_pred.getTypes();
				...
				// (A, ? , B) => use A+B
				{
					...
					String fullName = LSDConst.createFullMethodOrFieldName(
							...
							tk_binding_B.getGroundConst());
					LSDBinding tk_binding = LSDConst
							.createModifiedMethod(fullName).getBindings()
							.get(0);// create a new binding
					char[] types = tk_pred.getTypes();
					tk_binding.setType(types[0]);
