...
public class MethodStereotypeRules
{
    ...
    protected MethodStereotype checkForCollaborationalStereotype(final boolean asPrimaryStereotype) {
        ...
        for (final VariableInfo parameter : this.methodAnalyzer.getParameters()) {
            ...
            if (parameter.isReturned() && !parameter.getAssignedFields().isEmpty()) {
                ++returnedFieldVariables;
            }
            if (!parameter.isModified() || this.isPrimitive(parameter.getVariableBinding())) {
                continue;
            }
        ...
        if (asPrimaryStereotype) {
            if ((!this.methodAnalyzer.getParameters().isEmpty() && !allPrimitiveParameters) || (!this.methodAnalyzer.getVariables().isEmpty() && !allPrimitiveVariables)) {
                return MethodStereotype.COLLABORATOR;
            }
        }
        else if (((!this.methodAnalyzer.getParameters().isEmpty() && modifiedObjectParameters > 0) || (!this.methodAnalyzer.getVariables().isEmpty() && !allPrimitiveVariables)) && this.methodAnalyzer.getParameters().size() + this.methodAnalyzer.getVariables().size() > returnedFieldVariables) {
            return MethodStereotype.COLLABORATOR;
        }
    ...
    private boolean isVoid(final Type type) {
        if (type.isPrimitiveType()) {
            ...
            if (primitive.getPrimitiveTypeCode().equals((Object)PrimitiveType.VOID)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isBoolean(final Type type) {
        if (type.isPrimitiveType()) {
            final PrimitiveType primitive = (PrimitiveType)type;
            if (primitive.getPrimitiveTypeCode().equals((Object)PrimitiveType.BOOLEAN)) {
                return true;
            }
        }
        return false;
